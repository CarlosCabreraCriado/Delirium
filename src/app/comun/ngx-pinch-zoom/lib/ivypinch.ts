...
export class IvyPinch {
    ...
    handlePan = (event: any) => {
        ...
        /* mousemove */
        if (event.type === "mousemove" && this.scale > this.minPanScale && this.properties.limitPan) {
            this.centeringImage();
        }
    ...
    detectLimitZoom() {
        ...
        //Añadido Parche Limit Zoom:
        if(this.properties.limitZoom != "original image size"){
            this.maxScale = Number(this.properties.limitZoom);
        }
    ...
    setZoom(properties: {
        ...
        //CARLOS: Eliminado el auto centrado.
       // this.centeringImage();
        this.updateInitialValues();
        this.transformElement(this.properties.transitionDuration);
    }
